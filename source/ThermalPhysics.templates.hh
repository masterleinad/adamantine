    ...
    std::vector<Timer> &timers)
{
  ...
  if (print_heat_input)
  {
    ...
    {
      std::cout << "Time: " << t << " s, Global Heat Input: "
      ...
      _material_properties(material_properties)
{
  ...
  _print_heat_input = source_database.get<bool>("print_heat_input", false);
        ...
        std::vector<Timer> &timers)
{
  ...
  // perform a forward steps becomes costly. In that case, we just perform the
  ...
  else
  {
    if (_material_properties.properties_use_table())
    {
      return evaluate_thermal_physics_impl<dim, n_materials, true, p_order,
                                           ...
                                           MemorySpaceType>(
          _thermal_operator, _fe_collection, t, _dof_handler, _heat_sources,
          _print_heat_input, _boundary, _material_properties,
