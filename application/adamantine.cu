    ...
    std::vector<adamantine::Timer> &timers)
{
  ...
  thermal_physics->initialize_dof_vector(initial_temperature, solution_host);
  ...
  /*  if (((n_time_step % time_steps_refinement) == 0) ||
        (time >= next_refinement_time))
    {
      ...
      refine_mesh(thermal_physics, solution_host, electron_beams, time,
                  next_refinement_time, time_steps_refinement,
                  refinement_database, fe_degree);
      solution_device.reinit(solution_host.get_partitioner());
      solution_device.import(solution_host, dealii::VectorOperation::insert);
