...
template <int dim>
ArborX::ExperimentalHyperGeometry::Box<dim, double> to_arborx_box(dealii::BoundingBox<dim> dealii_box)
{
        ...
        &heat_sources)
{
  ...
  Kokkos::View<ArborX::ExperimentalHyperGeometry::Box<dim, double>*, Kokkos::HostSpace> heat_source_bounding_boxes("heat_source_bounding_boxes", n_time_steps * n_heat_sources);
  ...
  for (unsigned int i = 0; i < n_time_steps; ++i)
  {
    ...
    {
      heat_source_bounding_boxes[i*n_heat_sources+beam_idx] = to_arborx_box(heat_sources[beam_idx]->get_bounding_box(current_time, bounding_box_scaling));
    }
  }
  ArborX::BVH<Kokkos::HostSpace, ArborX::ExperimentalHyperGeometry::Box<dim, double>> tree(Kokkos::DefaultHostExecutionSpace{}, heat_source_bounding_boxes);

  // Build the bounding boxes associated with the locally owned cells
  int const n = triangulation.n_locally_owned_active_cells();
  Kokkos::View<decltype(ArborX::attach(ArborX::intersects(ArborX::ExperimentalHyperGeometry::Box<dim, double>{}),
                                       int{})) *,
               Kokkos::HostSpace>
  ...
  for (auto const &cell : triangulation.active_cell_iterators() |
    ...
    queries(i) =
        ArborX::attach(ArborX::intersects(to_arborx_box(cell->bounding_box())), i);
    ++i;
  }
  if (i!=n)
	  Kokkos::abort("bla");
